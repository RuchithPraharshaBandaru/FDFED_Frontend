...
import { adminLogin, managerLogin, checkAdminAuth, selectIsAdminAuthenticated, selectAdminAuthLoading, selectAdminAuthError } from '../../../store/slices/adminAuthSlice';
...
const AdminLoginPage = () => {
  ...
  const onSubmit = async (values) => {
    const res = await dispatch(role === 'manager' ? managerLogin(values) : adminLogin(values));
    ...
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-md space-y-6 p-6 rounded-2xl border bg-white/60 dark:bg-gray-900/50 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">{role === 'manager' ? 'Manager' : 'Admin'} Sign In</h1>
        </div>
        {error && (
