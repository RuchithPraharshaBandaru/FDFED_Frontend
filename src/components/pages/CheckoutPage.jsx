                            ...
                            {user.cart.map(item => (
                                item.productId ? ( // Check if product exists
                                    <div key={item.productId._id} className="flex justify-between items-center">
                                        <span className="text-gray-600 dark:text-gray-300">{item.productId.title} (x{item.quantity})</span>
                                        <span className="font-medium dark:text-white">${(item.productId.price * item.quantity).toFixed(2)}</span>
                                    </div>
                                ) : (
                                    <div key={item._id} className="flex justify-between items-center">
                                        <span className="text-gray-600 dark:text-gray-400 italic">Item no longer available</span>
                                    </div>
                                )
                            ))}
